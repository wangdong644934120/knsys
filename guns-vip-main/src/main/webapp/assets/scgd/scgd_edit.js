...
layui.use(['form', 'admin', 'ax','upload','laydate'], function () {
    ...
    //获取详情信息，填充表单
    var ajax = new $ax(Feng.ctxPath + "/scgd/detail?id=" + Feng.getUrlParam("id"));
    var result = ajax.start();
    form.val('scgdForm', result.data);
    /*跟单人*/
    var usersax=new $ax(Feng.ctxPath+"/knuser/getallusers");
    var users=usersax.start();
    for(var i=0;i<users.data.length;i++){
        $("#gdr").append("<option value='"+users.data[i].userId+"'>"+users.data[i].name+"</option>");
    }
    $("#gdr").val(result.data.gdrid);
    form.render();

    if($("#gdjg").val()!=''){
        $("#gdjg").attr("disabled","disabled");
        form.render("select");
    }

    // var pics=new $ax(Feng.ctxPath+"/scgd/getPic?tpzlId="+Feng.getUrlParam("id"));
    ...
    form.on('submit(btnSubmit)', function (data) {
        var ajax = new $ax(Feng.ctxPath + "/scgd/editItem", function (data) {
            Feng.success("更新成功！");
            window.location.href = Feng.ctxPath + '/scgd'
        }, function (data) {
    ...
    // upload.render({
    //     elem: '#test2'
    //     ,url: '/scgd/uploadPicture' //改成您自己的上传接口
    //     ,multiple: true
    //     ,before: function(obj){
    //         //预读本地文件示例，不支持ie8
    //
    ...
    //         var ajaxfile = new $ax(Feng.ctxPath + "/scgd/addPic?tpzlId=" + Feng.getUrlParam("id")+"&tplj="+res.data.fileUUID);
