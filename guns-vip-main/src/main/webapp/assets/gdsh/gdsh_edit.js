...
layui.use(['form', 'admin', 'ax','upload','laydate'], function () {
    ...
    //获取详情信息，填充表单
    var ajax = new $ax(Feng.ctxPath + "/gdsh/detail?id=" + Feng.getUrlParam("id"));
    var result = ajax.start();
    form.val('gdshForm', result.data);
    /*跟单人*/
    var usersax=new $ax(Feng.ctxPath+"/knuser/getallusers");
    var users=usersax.start();
    for(var i=0;i<users.data.length;i++){
        $("#gdshr").append("<option value='"+users.data[i].userId+"'>"+users.data[i].name+"</option>");
    }
    $("#gdshr").val(result.data.gdshrid);
    form.render();

    // var pics=new $ax(Feng.ctxPath+"/gdsh/getPic?tpzlId="+Feng.getUrlParam("id"));
    ...
    form.on('submit(btnSubmitJXGD)', function (data) {
        var ajax = new $ax(Feng.ctxPath + "/gdsh/jxgd", function (data) {
            Feng.success("更新成功！");
            window.location.href = Feng.ctxPath + '/gdsh'
        }, function (data) {
    ...
    form.on('submit(btnSubmitFQGD)', function (data) {
        var ajax = new $ax(Feng.ctxPath + "/gdsh/qrfq", function (data) {
            Feng.success("更新成功！");
            window.location.href = Feng.ctxPath + '/gdsh'
        }, function (data) {
            Feng.error("更新失败！" + data.responseJSON.message)
    ...
    // upload.render({
    //     elem: '#test2'
    //     ,url: '/gdsh/uploadPicture' //改成您自己的上传接口
    //     ,multiple: true
    //     ,before: function(obj){
    //         //预读本地文件示例，不支持ie8
    //
    ...
    //         var ajaxfile = new $ax(Feng.ctxPath + "/gdsh/addPic?tpzlId=" + Feng.getUrlParam("id")+"&tplj="+res.data.fileUUID);
