...
layui.use(['form', 'admin', 'ax','upload','laydate'], function () {
    ...
    //获取详情信息，填充表单
    var ajax = new $ax(Feng.ctxPath + "/khcx/detail?id=" + Feng.getUrlParam("id"));
    var result = ajax.start();
    form.val('khcxForm', result.data);
    /*跟单人*/
    var usersax=new $ax(Feng.ctxPath+"/knuser/getallusers");
    var users=usersax.start();
    for(var i=0;i<users.data.length;i++){
        $("#khcxr").append("<option value='"+users.data[i].userId+"'>"+users.data[i].name+"</option>");
    }
    $("#khcxr").val(result.data.khcxrid);
    form.render();

    // var pics=new $ax(Feng.ctxPath+"/khcx/getPic?tpzlId="+Feng.getUrlParam("id"));
    ...
    //表单提交事件
    form.on('submit(btnSubmitJXGD)', function (data) {
        var ajax = new $ax(Feng.ctxPath + "/khcx/jxgd", function (data) {
            Feng.success("更新成功！");
            window.location.href = Feng.ctxPath + '/khcx'
        }, function (data) {
    ...
    form.on('submit(btnSubmitFQGD)', function (data) {
        var ajax = new $ax(Feng.ctxPath + "/khcx/qrfq", function (data) {
            Feng.success("更新成功！");
            window.location.href = Feng.ctxPath + '/khcx'
        }, function (data) {
            Feng.error("更新失败！" + data.responseJSON.message)
    ...
    // upload.render({
    //     elem: '#test2'
    //     ,url: '/khcx/uploadPicture' //改成您自己的上传接口
    //     ,multiple: true
    //     ,before: function(obj){
    //         //预读本地文件示例，不支持ie8
    //
    ...
    //         var ajaxfile = new $ax(Feng.ctxPath + "/khcx/addPic?tpzlId=" + Feng.getUrlParam("id")+"&tplj="+res.data.fileUUID);
