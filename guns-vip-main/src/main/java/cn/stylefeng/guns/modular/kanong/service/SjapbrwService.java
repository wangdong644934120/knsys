...
import cn.stylefeng.guns.modular.kanong.mapper.SjapbrwMapper;
 ...
 */
@Service
public class SjapbrwService extends ServiceImpl<SjapbrwMapper, Ddkh> {
    ...
    public DdkhResult getSjapbById(DdkhParam param){
        List<DdkhResult> list=this.baseMapper.getSjapbrwById(param);
        if(list!=null && !list.isEmpty()){
            return list.get(0);
        }else{
            return null;
