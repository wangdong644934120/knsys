...
import cn.stylefeng.guns.modular.kanong.service.PmsjrwService;
 ...
 */
@Controller
@RequestMapping("/pmsjrw")
public class PmsjrwController extends BaseController {

    private String PREFIX = "/pmsjrw";
    @Autowired
    private PmsjrwService pmsjrwService;
    @Autowired
    ...
    @RequestMapping("/edit")
    public String edit() {
        return PREFIX + "/pmsjrw_edit.html";
    }
    ...
    public ResponseData editItem(DdkhParam ddkhParam) {
        this.pmsjrwService.updateRW(ddkhParam);
        if(ddkhParam.getPmsjwczt().equals("完成")){
            //发送通知
    ...
    public ResponseData detail(DdkhParam ddkhParam) {
        Ddkh detail = this.pmsjrwService.getPmsjrwById(ddkhParam);
    ...
    public LayuiPageInfo list(DdkhParam ddkhParam) {
        return this.pmsjrwService.findPageBySpec(ddkhParam);
    }
}
