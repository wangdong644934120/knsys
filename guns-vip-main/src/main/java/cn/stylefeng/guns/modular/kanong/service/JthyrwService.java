 ...
 */
@Service
public class JthyrwService extends ServiceImpl<JthyrwMapper, Ddkh> {
    ...
    public DdkhResult getJthyrwById(DdkhParam param){
        List<DdkhResult> list=this.baseMapper.getJthyrwById(param);
        if(list!=null && !list.isEmpty()){
            return list.get(0);
        }else{
            return null;
