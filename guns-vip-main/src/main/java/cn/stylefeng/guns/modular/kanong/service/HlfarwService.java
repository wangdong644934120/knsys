...
import cn.stylefeng.guns.modular.kanong.mapper.HlsysMapper;
 ...
 */
@Service
public class HlfarwService extends ServiceImpl<HlfarwMapper, Ddkh> {
    ...
    public DdkhResult getHlfarwById(DdkhParam param){
        List<DdkhResult> list=this.baseMapper.getHlfarwById(param);
        if(list!=null && !list.isEmpty()){
            return list.get(0);
        }else{
            return null;
    ...
    public int updateXGT(DdkhParam param){
        return this.baseMapper.updateXGT(param);
    }

    public int updateHTQD(DdkhParam param){
        return this.baseMapper.updateHTQD(param);
    }
}
