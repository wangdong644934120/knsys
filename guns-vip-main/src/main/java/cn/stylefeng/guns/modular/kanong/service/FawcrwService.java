...
import cn.stylefeng.guns.modular.kanong.mapper.FawcrwMapper;
 ...
 */
@Service
public class FawcrwService extends ServiceImpl<FawcrwMapper, Ddkh> {
    ...
    public DdkhResult getFawcrwById(DdkhParam param){
        List<DdkhResult> list=this.baseMapper.getFawcrwById(param);
        if(list!=null && !list.isEmpty()){
            return list.get(0);
        }else{
            return null;
