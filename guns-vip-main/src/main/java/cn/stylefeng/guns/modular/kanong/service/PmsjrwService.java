...
import cn.stylefeng.guns.modular.kanong.mapper.PmsjrwMapper;
 ...
 */
@Service
public class PmsjrwService extends ServiceImpl<PmsjrwMapper, Ddkh> {
    ...
    public DdkhResult getPmsjrwById(DdkhParam param){
        List<DdkhResult> list=this.baseMapper.getPmsjrwById(param);
        if(list!=null && !list.isEmpty()){
            return list.get(0);
        }else{
            return null;
