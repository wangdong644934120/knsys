...
import cn.stylefeng.guns.modular.kanong.service.DjhyrwService;
 ...
 */
@Controller
@RequestMapping("/djhyrw")
public class DjhyrwController extends BaseController {

    private String PREFIX = "/djhyrw";
    @Autowired
    private DjhyrwService djhyrwService;
    @Autowired
    ...
    @RequestMapping("/edit")
    public String edit() {
        return PREFIX + "/djhyrw_edit.html";
    }
    ...
    public ResponseData editItem(DdkhParam ddkhParam) {
        this.djhyrwService.updateRW(ddkhParam);
        if(ddkhParam.getDjhywczt().equals("完成")){
            //发送通知
    ...
    public ResponseData detail(DdkhParam ddkhParam) {
        Ddkh detail = this.djhyrwService.getDjhyrwById(ddkhParam);
    ...
    public LayuiPageInfo list(DdkhParam ddkhParam) {
        return this.djhyrwService.findPageBySpec(ddkhParam);
    }
}
