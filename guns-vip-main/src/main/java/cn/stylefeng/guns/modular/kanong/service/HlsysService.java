...
import cn.stylefeng.guns.modular.kanong.model.params.HlsysParam;
import cn.stylefeng.guns.modular.kanong.model.result.DdkhResult;
import cn.stylefeng.guns.modular.kanong.model.result.HlsysResult;
 ...
 */
@Service
public class HlsysService extends ServiceImpl<HlsysMapper, Ddkh> {
    ...
    public DdkhResult getHlsysById(DdkhParam param){
        List<DdkhResult> list=this.baseMapper.getHlsysById(param);
        if(list!=null && !list.isEmpty()){
            return list.get(0);
        }else{
            return null;
