...
import cn.stylefeng.guns.modular.kanong.service.HchrwService;
 ...
 */
@Controller
@RequestMapping("/hchrw")
public class HchrwController extends BaseController {

    private String PREFIX = "/hchrw";
    @Autowired
    private HchrwService hchrwService;
    ...
    public ResponseData editItem(DdkhParam ddkhParam) {
        this.hchrwService.updateRW(ddkhParam);
        if(ddkhParam.getHchwczt().equals("完成")){
            //发送通知
    ...
    public ResponseData detail(DdkhParam ddkhParam) {
        Ddkh detail = this.hchrwService.getHchrwById(ddkhParam);
    ...
    public LayuiPageInfo list(DdkhParam ddkhParam) {
        return this.hchrwService.findPageBySpec(ddkhParam);
    }
}
