...
import cn.stylefeng.guns.modular.kanong.mapper.HlsyMapper;
 ...
 */
@Service
public class HlsyService extends ServiceImpl<HlsyMapper, Ddkh> {
    ...
    public DdkhResult getHlsyById(DdkhParam param){
        List<DdkhResult> list=this.baseMapper.getHlsyById(param);
        if(list!=null && !list.isEmpty()){
            return list.get(0);
        }else{
            return null;
