...
import cn.stylefeng.guns.modular.kanong.service.WlcgrwService;
 ...
 */
@Controller
@RequestMapping("/wlcgrw")
public class WlcgrwController extends BaseController {

    private String PREFIX = "wlcgrw";
    @Autowired
    private WlcgrwService wlcgrwService;
    @Autowired
    ...
    @RequestMapping("/edit")
    public String edit() {
        return PREFIX + "/wlcgrw_edit.html";
    }
    ...
    public ResponseData editItem(DdkhParam ddkhParam) {
        this.wlcgrwService.updateRW(ddkhParam);
        if(ddkhParam.getWlcgwczt().equals("完成")){
            //发送通知
    ...
    public ResponseData detail(DdkhParam ddkhParam) {
        Ddkh detail = this.wlcgrwService.getWlcgrwById(ddkhParam);
    ...
    public LayuiPageInfo list(DdkhParam ddkhParam) {
        return this.wlcgrwService.findPageBySpec(ddkhParam);
    }
