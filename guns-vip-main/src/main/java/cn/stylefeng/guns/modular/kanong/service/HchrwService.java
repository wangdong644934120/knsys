...
import cn.stylefeng.guns.modular.kanong.mapper.HchrwMapper;
 ...
 */
@Service
public class HchrwService extends ServiceImpl<HchrwMapper, Ddkh> {
    ...
    public DdkhResult getHchrwById(DdkhParam param){
        List<DdkhResult> list=this.baseMapper.getHchrwById(param);
        if(list!=null && !list.isEmpty()){
            return list.get(0);
        }else{
            return null;
