...
import cn.stylefeng.guns.modular.kanong.service.HlsyService;
...
@Controller
@RequestMapping("/hlsy")
public class HlsyController extends BaseController {

    private String PREFIX = "/hlsy";

    @Autowired
    private HlsyService hlsyService;
    ...
    public ResponseData editItem(DdkhParam ddkhParam) {
        this.hlsyService.updateRW(ddkhParam);
        if(ddkhParam.getSywczt().equals("完成")){
            //发送通知
            ...
            if(listUsers!=null && !listUsers.isEmpty()){
                for(User useropen : listUsers){
                    ...
                    message.setKeyword2("司仪配置完成:"+ddkhParam.getSypz());
                    list.add(message);
                }
    ...
    public ResponseData detail(DdkhParam ddkhParam) {
        Ddkh detail = this.hlsyService.getHlsyById(ddkhParam);
    ...
    public LayuiPageInfo list(DdkhParam ddkhParam) {
        return this.hlsyService.findPageBySpec(ddkhParam);
    }
