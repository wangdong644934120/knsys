...
import cn.stylefeng.guns.modular.kanong.service.DzqtrwService;
 ...
 */
@Controller
@RequestMapping("/dzqtrw")
public class DzqtrwController extends BaseController {

    private String PREFIX = "dzqtrw";
    @Autowired
    private DzqtrwService dzqtrwService;
    @Autowired
    ...
    @RequestMapping("/edit")
    public String edit() {
        return PREFIX + "/dzqtrw_edit.html";
    }
    ...
    public ResponseData editItem(DdkhParam ddkhParam) {
        this.dzqtrwService.updateRW(ddkhParam);
        if(ddkhParam.getDzqtwczt().equals("完成")){
            //发送通知
    ...
    public ResponseData detail(DdkhParam ddkhParam) {
        Ddkh detail = this.dzqtrwService.getDzqtrwById(ddkhParam);
    ...
    public LayuiPageInfo list(DdkhParam ddkhParam) {
        return this.dzqtrwService.findPageBySpec(ddkhParam);
    }
