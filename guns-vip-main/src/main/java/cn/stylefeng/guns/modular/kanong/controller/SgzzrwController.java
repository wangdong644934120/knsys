...
import cn.stylefeng.guns.modular.kanong.service.SgzzrwService;
 ...
 */
@Controller
@RequestMapping("/sgzzrw")
public class SgzzrwController extends BaseController {

    private String PREFIX = "/sgzzrw";
    @Autowired
    private SgzzrwService sgzzrwService;
    @Autowired
    ...
    @RequestMapping("/edit")
    public String edit() {
        return PREFIX + "/sgzzrw_edit.html";
    }
    ...
    public ResponseData editItem(DdkhParam ddkhParam) {
        this.sgzzrwService.updateRW(ddkhParam);
        if(ddkhParam.getSgzzwczt().equals("完成")){
            //发送通知
    ...
    public ResponseData detail(DdkhParam ddkhParam) {
        Ddkh detail = this.sgzzrwService.getSgzzrwById(ddkhParam);
    ...
    public LayuiPageInfo list(DdkhParam ddkhParam) {
        return this.sgzzrwService.findPageBySpec(ddkhParam);
    }
}
